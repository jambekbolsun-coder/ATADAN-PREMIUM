...
export async function visibleDeal(actor: Actor, id: string) {
  const deal = await getRawDb().prepare("SELECT * FROM crm_deals WHERE id=? AND archived=0").bind(id).first<{id:string; title:string;tractor_slug:string|null;assigned_to:string|null;stage:string;amount_minor:number;cost_minor:number|null;version:number;customer_id:string;lead_id:string|null;inventory_unit_id:string|null;probability:number|null;next_step_at:string|null}>();
