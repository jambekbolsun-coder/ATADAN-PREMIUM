...
type DashboardData = {
  ...
  director?: {
    ...
    modelRegions?:Array<{tractor_slug:string;region:string;views:number}>;
...
const workspaceSections:Record<AdminWorkspace,SectionDef[]>={
  ...
  control:[
    {id:"overview",label:"Рабочий стол",icon:LayoutDashboard},{id:"control-plan",label:"План / факт",icon:Gauge,roles:["owner","director"]},{id:"control-models",label:"Аналитика моделей",icon:Tractor,roles:["owner","director"]},{id:"control-demand",label:"Спрос и регионы",icon:MapPinned,roles:["owner","director"]},{id:"control-attribution",label:"Сквозная аналитика",icon:BarChart3,roles:["owner","director"]},{id:"control-ai",label:"Аналитический помощник",icon:Bot,roles:["owner","director"]},{id:"profile",label:"Мой профиль",icon:Settings},
  ],
};
...
function InventoryCatalog({data}:{data:DashboardData}){
  ...
  return <div className="inventory-center"><section className="admin-content"><div className="admin-panel inventory-catalog-panel"><div className="records-intro inventory-heading"><div><span>Единый каталог</span><h2>Все товары уже связаны со складом</h2><p>Карточка товара используется на публичном сайте и в складском учёте. Для физической единицы добавьте VIN и статус ниже.</p></div><label><Search aria-hidden="true"/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Модель, мощность или категория" aria-label="Поиск по складскому каталогу"/></label></div><div className="inventory-catalog-grid">{products.map(product=><article key={product.slug}><span><Image src={product.image} alt={`Changfa ${product.model}`} width={150} height={105}/></span><div><small>{product.category}</small><h3>Changfa {product.model}</h3><p>{product.hp} л.с. · {product.inStock?"доступен к продаже":"под заказ"}</p></div><strong>≈ {new Intl.NumberFormat("ru-RU").format(product.approximatePriceUsd ?? 10000)} $<small> ориентир</small></strong></article>)}</div>{!products.length?<div className="admin-empty"><Search/><h3>Товары не найдены</h3><p>Измените поисковый запрос.</p></div>:null}</div></section><AdminRecordsManager kind="inventory_units"/></div>;
}
...
function DirectorDetail({section,data,onNavigate}:{section:string;data:DashboardData;onNavigate:(section:string)=>void}){
  const director=data.director??{};const operations=director.operations;const models=director.models??[];const channels=director.channels??[];const managers=director.managers??[];
  const money=(minor:number|undefined)=>new Intl.NumberFormat("ru-RU").format(Math.round((Number(minor)||0)/100))+" сом";
  const periodLeads=director.period?.leads_30??0,periodSales=director.comparison?.sales_current??0;
  const conversion=periodLeads>0?Math.round(periodSales/periodLeads*1000)/10:0;
  if(section==="control-plan")return <div className="admin-content"><section className="admin-panel director-page"><PageHeading title="План / факт · 30 дней" text="Продажи, выручка, прибыль, лиды, встречи и конверсия рассчитаны за один текущий 30-дневный период."/><div className="director-facts"><Fact label="Продажи · 30 дней" fact={periodSales} plan={director.goals?.sales??0}/><Fact label="Выручка · 30 дней" fact={Math.round((director.comparison?.revenue_current_minor??0)/100)} plan={Math.round((director.goals?.revenueMinor??0)/100)} money/><Fact label="Прибыль · 30 дней" fact={Math.round((director.comparison?.profit_current_minor??0)/100)} plan={Math.round((director.goals?.profitMinor??0)/100)} money/><Fact label="Лиды · 30 дней" fact={periodLeads} plan={director.goals?.leads??0}/><Fact label="Встречи · 30 дней" fact={operations?.meetings_30??0} plan={director.goals?.meetings??0}/><Fact label="Конверсия · 30 дней, %" fact={conversion} plan={director.goals?.conversion??0}/></div></section></div>;
  if(section==="control-models")return <><DirectorTable title="Аналитика моделей" text="Просмотр не равен спросу: показана полная последовательная воронка." heads={["Модель","Просмотры","Лиды","Квалификация","Встречи","КП","Продажи","Выручка","Прибыль"]} rows={models.map(item=>[data.catalog.find(product=>product.slug===item.tractor_slug)?.model??item.tractor_slug,item.views,item.leads,item.qualified,item.meetings,item.proposals,item.sales,money(item.revenue_minor),money(item.profit_minor)])}/><DirectorTable title="Просмотры моделей по регионам" text="Регион выбран посетителем в квизе. Просмотры учитываются при согласии на аналитику; старые просмотры остаются без региона." heads={["Модель","Регион","Просмотры"]} rows={(director.modelRegions??[]).map(item=>[data.catalog.find(p=>p.slug===item.tractor_slug)?.model??item.tractor_slug,item.region,item.views])}/></>;
...
function ProductEditor({ product, close, save }: { product: TractorType; close: () => void; save: (product: TractorType) => Promise<boolean>|boolean }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const saved=await save({
      ...
      approximatePriceUsd: Number(f.get("approximatePriceUsd")),
