...
const ui={
  ru:{title:"Подберём трактор за минуту",text:"Ответьте на два коротких вопроса, и мы покажем подходящую мощность.",start:"Начать подбор",later:"Не сейчас",area:"Какая площадь хозяйства?",work:"Какие работы главные?",priority:"Что важнее всего?",back:"Назад",result:"Подходящий диапазон",show:"Показать модели",faq:"Помощник ATADAN",faqText:"Короткие ответы перед разговором с менеджером.",q:["Есть гарантия?","Можно оформить лизинг?","Есть сервис и запчасти?"],a:["Да, условия гарантии фиксируются для выбранной модели и комплектации.","Да. Калькулятор показывает предварительный график на 7 лет, финальные условия подтверждает финансовый партнёр.","Да. ATADAN помогает с обслуживанием техники и подбором запасных частей."],ask:"Написать менеджеру"},
  ky:{title:"Тракторду бир мүнөттө тандайбыз",text:"Эки кыска суроого жооп бериңиз, биз ылайыктуу кубаттуулукту көрсөтөбүз.",start:"Тандоону баштоо",later:"Азыр эмес",area:"Чарбанын аянты канча?",work:"Негизги жумуш кайсы?",priority:"Эмнеси маанилүү?",back:"Артка",result:"Ылайыктуу диапазон",show:"Моделдерди көрүү",faq:"ATADAN жардамчысы",faqText:"Менеджерге чейин кыска жооптор.",q:["Кепилдик барбы?","Лизинг барбы?","Сервис жана тетиктер барбы?"],a:["Ооба, кепилдик шарттары тандалган модель жана комплектация үчүн бекитилет.","Ооба. Калькулятор 7 жылдык болжолдуу графикти көрсөтөт, акыркы шарттарды каржы өнөктөшү бекитет.","Ооба. ATADAN тейлөөгө жана запастык бөлүктөрдү тандоого жардам берет."],ask:"Менеджерге жазуу"},
  en:{title:"Find your tractor in a minute",text:"Answer two short questions and see a suitable power range.",start:"Start selection",later:"Not now",area:"How large is the farm?",work:"What is the main job?",priority:"What matters most?",back:"Back",result:"Suggested power range",show:"View models",faq:"ATADAN assistant",faqText:"Quick answers before you speak with a manager.",q:["Is there a warranty?","Is leasing available?","Do you provide service and parts?"],a:["Yes. Warranty terms are confirmed for the selected model and configuration.","Yes. The calculator shows a preliminary schedule over 7 years; final terms come from the finance partner.","Yes. ATADAN helps with machinery service and parts selection."],ask:"Message a manager"}
} as const;
...
export function SiteAssist({faqs=[]}:{faqs?:SiteFaq[]}){
  ...
  return <>
    <div className="site-assist">
      ...
      <button className="assist-bot" type="button" aria-label={l.faq} aria-expanded={assistant} aria-controls="atadan-assistant" onClick={()=>setAssistant(v=>!v)}><span className="assist-bot-symbol"><Bot aria-hidden="true"/><Sparkles aria-hidden="true"/></span></button>
    </div>
    {quiz?<div className="quiz-overlay"><button className="quiz-backdrop" tabIndex={-1} type="button" onClick={closeQuiz} aria-label={l.later}/><section ref={quizRef} className="tractor-quiz" role="dialog" aria-modal="true" aria-labelledby="quiz-title"><button className="quiz-close" type="button" onClick={closeQuiz} aria-label={l.later}><X/></button><span className="quiz-icon"><Sparkles/></span><div className="quiz-progress" aria-label={`Вопрос ${step+1} из 2`}><i style={{width:`${(step+1)*50}%`}}/></div><p>Вопрос {step+1} из 2</p><h2 id="quiz-title">{step===0?"Какой мощности трактор вам нужен?":"Из какого вы региона?"}</h2><div className="quiz-options">{step===0?powerRanges.map((range,index)=><button type="button" key={range.label} onClick={()=>{setPowerIndex(index);setStep(1)}}>{range.label}<ChevronRight/></button>):regions.map(region=><button type="button" key={region} onClick={()=>{sessionStorage.setItem("atadan-region",region);window.dispatchEvent(new Event("atadan:region"));closeQuiz();router.push(`/catalog?power=${powerRanges[powerIndex].query}`)}}>{region}<ChevronRight/></button>)}</div>{step===1?<button className="quiz-later" type="button" onClick={()=>setStep(0)}>{l.back}</button>:<button className="quiz-later" type="button" onClick={closeQuiz}>{l.later}</button>}</section></div>:null}
  </>;
}
