...
export function AdminPortalHome({actor,onChoose,onLogout}:{actor:{display_name:string;role:"owner"|"manager";avatar:string|null};onChoose:(workspace:AdminWorkspace)=>void;onLogout:()=>void}){
  return <main className="admin-portal-home">
    ...
    <section className="admin-portal-choices" aria-label="Рабочие пространства">{portals.map(({id,title,subtitle,Icon,accent})=>{
      const allowed=actor.role==="owner"||id==="company";
