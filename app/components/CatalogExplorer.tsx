...
export function CatalogExplorer({
  ...
  const [range, setRange] = useState(() => {
    ...
    const found = ranges.findIndex(
      (item, index) =>
        index > 0 && initialPower >= item.min && initialPower <= item.max,
    );
  ...
  const filtered = useMemo(
    () =>
      tractors
        .filter((tractor) => {
          const matchesQuery = tractor.model
            .toLowerCase()
            .includes(deferredQuery.toLowerCase());
          const markedPopular = tractors.some((item) => item.popular);
          const matchesPopular =
            !popularOnly ||
            (markedPopular ? tractor.popular : tractor.hp >= 200);
          return (
            matchesQuery &&
            matchesPopular &&
            tractor.hp >= selected.min &&
            tractor.hp <= selected.max &&
            (!stockOnly || tractor.inStock)
          );
        })
  ...
  const renderFilters = (scope: "desktop" | "mobile") => (
    <>
      ...
      <fieldset>
        <legend>{t("catalog.power")}</legend>
        {ranges.map((item, index) => (
          <label className="radio-row" key={item.label}>
            <input
              type="radio"
              name={`power-${scope}`}
              checked={range === index}
              onChange={() => setRange(index)}
            />
            <span>{index === 0 ? t("catalog.allPower") : item.label}</span>
          </label>
        ))}
      </fieldset>
      <fieldset>
        <legend>{t("catalog.availability")}</legend>
