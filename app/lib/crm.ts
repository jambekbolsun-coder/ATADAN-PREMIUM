...
export const stageLabels: Record<Stage,string> = {new:"Новая",contacted:"В работе",proposal:"Предложение",negotiation:"Согласование",won:"Продано",lost:"Отказ"};
export function canTransition(from: string, to: string, role: string, reason: string, amount: number) {
  ...
  if ((from === "won" || from === "lost") && from !== to && role !== "owner") throw new HttpError(403,"Закрытую сделку может вернуть управляющий");
...
export async function visibleDeal(actor: Actor, id: string) {
  ...
  if (!deal || (actor.role !== "owner" && deal.assigned_to !== actor.id)) throw new HttpError(404,"Сделка не найдена");
  return deal;
}
