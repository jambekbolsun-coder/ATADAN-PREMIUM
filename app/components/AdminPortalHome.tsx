...
export function AdminPortalHome({actor,onChoose,onLogout}:{actor:{display_name:string;role:keyof typeof roleLabels;avatar:string|null};onChoose:(workspace:AdminWorkspace)=>void;onLogout:()=>void}){
  return <main className="admin-portal-home">
    ...
    <header className="admin-portal-header"><div className="admin-portal-brand"><Image className="premium-brand-image" src="/atadan-premium-logo.png" alt="ATADAN, официальный дистрибьютор Changfa" width={1254} height={1254} priority/><span>единая система управления</span></div><div className="portal-account"><span><UserRound aria-hidden="true"/></span><div><strong>{actor.display_name}</strong><small>{roleLabels[actor.role]}</small></div><button type="button" onClick={onLogout} aria-label="Выйти"><LogOut/></button></div></header>
