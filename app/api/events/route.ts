...
export async function POST(request: Request) {
  try {
    ...
    const db=getRawDb();
    await db.batch([
      db.prepare("INSERT INTO interest_events (id, tractor_slug, path, event_type, visitor_id) VALUES (?, ?, ?, ?, ?)").bind(crypto.randomUUID(), body.tractorSlug ?? null, path, eventType, body.visitorId?.slice(0, 80) ?? null),
