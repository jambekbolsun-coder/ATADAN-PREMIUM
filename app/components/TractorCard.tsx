  ...
  return (
    <article className={`tractor-card ${featured ? "featured" : ""}`}>
      ...
      <div className="tractor-card-body">
        <div className="tractor-kicker"><span>{tractor.category}</span><span className={discount && tractor.price ? "sale-card-price" : "tractor-public-price"}>{discount && tractor.price ? <del>{formatPrice(tractor.price)}</del> : null}{salePrice ? formatPrice(salePrice) : formatApproximateUsdPrice(tractor.approximatePriceUsd)}{!salePrice && tractor.approximatePriceUsd ? <small>ориентировочно</small> : null}</span></div>
