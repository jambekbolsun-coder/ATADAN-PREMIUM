...
export async function POST(request:Request){
  try{
    ...
    const invite=await db.prepare("SELECT email,role,permissions_json FROM staff_invites WHERE token_hash=? AND used_at IS NULL AND revoked=0 AND expires_at>?").bind(hash,now).first<{email:string;role:"manager"|"accountant"|"marketer";permissions_json:string}>();
    ...
    const result=await db.batch([
      db.prepare("INSERT INTO staff(id,email,display_name,password_hash,salt,role,permissions_json) SELECT ?,email,?,?,?,role,permissions_json FROM staff_invites WHERE token_hash=? AND used_at IS NULL AND revoked=0 AND expires_at>?").bind(id,name,derived.hash,derived.salt,hash,now),
