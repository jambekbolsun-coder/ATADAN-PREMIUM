...
type Deal = { id:string; title:string; tractor_slug:string|null; stage:string; amount_minor:number; cost_minor?:number|null; assigned_to:string|null; loss_reason:string; loss_reason_code:string; inventory_unit_id:string|null; probability:number|null; next_step_at:string|null; version:number; name:string; phone:string; updated_at:string };
...
type Task = { id:string; deal_id:string|null; customer_id:string|null; title:string; description:string;priority:"low"|"normal"|"high"|"urgent";assigned_to:string;assigned_name:string;deal_title:string|null;customer_name:string|null; due_at:string; reminder_at:string|null; status:string; done:number; version:number };
...
type InventoryUnit={id:string;vin:string;model:string;status:string;reserved_deal_id:string|null;list_price_minor:number};
type TimelineEvent={id:string;type:string;title:string;detail:string;created_at:string};
type CrmData = { actor:Actor; deals:Deal[]; customers:Customer[]; tasks:Task[]; staff:Staff[]; costs:Cost[]; audit:Audit[]; notes:Note[]; inventory:InventoryUnit[]; stages:StageOption[] };
...
function Deals({data,catalog,action}:{data:CrmData;catalog:Tractor[];action:(p:Record<string,unknown>)=>Promise<unknown>}) {
  ...
  async function moveDeal(deal:Deal,target:string){
    ...
    await action({action:"move_deal",id:deal.id,version:deal.version,stage:target});
  }
...
function DealCard({deal,data,stages,action,move}:{deal:Deal;data:CrmData;stages:StageOption[];action:(p:Record<string,unknown>)=>Promise<unknown>;move:(stage:string)=>void}){
  ...
  const [lossCode,setLossCode]=useState(deal.loss_reason_code||"");
  const [note,setNote]=useState("");
  async function update(event:FormEvent<HTMLFormElement>){event.preventDefault();const f=new FormData(event.currentTarget);await action({action:"update_deal",id:deal.id,version:deal.version,stage:f.get("stage"),amount:Number(f.get("amount")||0),assignedTo:f.get("assignedTo"),inventoryUnitId:f.get("inventoryUnitId"),probability:Number(f.get("probability")||0),nextStepAt:f.get("nextStepAt"),lossReasonCode:lossCode,lossReason:loss});}
  ...
  return <article className={"crm-deal stage-"+deal.stage}>
    ...
    <details className="crm-deal-details"><summary>Открыть карточку</summary><form onSubmit={update}>
      ...
      <label><span>Вероятность, %</span><input name="probability" type="number" min="0" max="100" defaultValue={deal.probability??0}/></label>
      <label><span>Следующий шаг</span><input name="nextStepAt" type="datetime-local" defaultValue={deal.next_step_at?new Date(deal.next_step_at).toISOString().slice(0,16):""}/></label>
      <label><span>Конкретный VIN</span><select name="inventoryUnitId" defaultValue={deal.inventory_unit_id??""}><option value="">Не выбран</option>{data.inventory.filter(unit=>!["sold","delivered"].includes(unit.status)&&(!unit.reserved_deal_id||unit.reserved_deal_id===deal.id)).map(unit=><option key={unit.id} value={unit.id}>{unit.vin} · {unit.model} · {unit.status}</option>)}</select></label>
      ...
      <label><span>Категория отказа</span><select value={lossCode} onChange={event=>setLossCode(event.target.value)}><option value="">Выберите при отказе</option><option value="price">Цена</option><option value="financing">Финансирование</option><option value="competitor">Выбран конкурент</option><option value="timing">Не готов сейчас</option><option value="no_contact">Нет связи</option><option value="model">Не подошла модель</option><option value="duplicate">Дубликат</option><option value="other">Другое</option></select></label>
      <label><span>Пояснение отказа</span><input value={loss} onChange={e=>setLoss(e.target.value)} placeholder="Обязательно для этапа «Закрыто»"/></label>
    ...
    </form><HistoryPanel entityType="deal" entityId={deal.id}/></details>
  </article>;
}
...
function CustomerForm({customer,action}:{customer:Customer;action:(p:Record<string,unknown>)=>Promise<unknown>}){async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();const f=new FormData(event.currentTarget);await action({action:"update_customer",id:customer.id,version:customer.version,name:f.get("name"),phone:f.get("phone"),email:f.get("email"),region:f.get("region"),source:f.get("source"),tractorSlug:f.get("tractorSlug"),power:f.get("power"),purpose:f.get("purpose"),farmArea:f.get("farmArea"),budget:Number(f.get("budget")||0),purchaseMethod:f.get("purchaseMethod"),purchaseTiming:f.get("purchaseTiming"),notes:f.get("notes")});}return <><form className="crm-customer-form" onSubmit={submit}>{[["name","ФИО",customer.name],["phone","Телефон",customer.phone],["email","Email",customer.email],["region","Регион",customer.region],["source","Источник",customer.source],["tractorSlug","Модель",customer.tractor_slug??""],["power","Мощность, л.с.",customer.power??""],["purpose","Назначение техники",customer.purpose],["farmArea","Площадь хозяйства",customer.farm_area],["budget","Бюджет, сом",Math.round(customer.budget_minor/100)||""],["purchaseMethod","Способ покупки",customer.purchase_method],["purchaseTiming","Срок покупки",customer.purchase_timing]].map(([name,label,value])=><label key={String(name)}><span>{label}</span><input name={String(name)} defaultValue={String(value)} required={name==="name"||name==="phone"} type={["power","budget"].includes(String(name))?"number":"text"}/></label>)}<label className="full"><span>Заметки</span><textarea name="notes" rows={4} defaultValue={customer.notes}/></label><button className="admin-primary" type="submit">Сохранить карточку</button></form><HistoryPanel entityType="customer" entityId={customer.id}/></>}

function HistoryPanel({entityType,entityId}:{entityType:"customer"|"deal";entityId:string}){const [events,setEvents]=useState<TimelineEvent[]|null>(null),[error,setError]=useState(""),[open,setOpen]=useState(false);async function toggle(){const next=!open;setOpen(next);if(!next||events)return;setError("");try{const response=await fetch(`/api/admin/crm?mode=timeline&entityType=${entityType}&entityId=${encodeURIComponent(entityId)}`,{cache:"no-store"});const body=await response.json() as {events?:TimelineEvent[];error?:string};if(!response.ok)throw new Error(body.error||"История недоступна");setEvents(body.events??[])}catch(cause){setError(cause instanceof Error?cause.message:"История недоступна")}}return <section className="crm-entity-history"><button type="button" onClick={()=>void toggle()}><Clock3 size={15}/>{open?"Скрыть полную историю":"Показать полную историю"}</button>{open?<div>{error?<p className="crm-message error">{error}</p>:events===null?<p>Загружаем историю…</p>:events.length?events.map(item=><article key={`${item.type}-${item.id}`}><time>{new Date(item.created_at).toLocaleString("ru-RU")}</time><strong>{item.title}</strong>{item.detail?<p>{item.detail}</p>:null}</article>):<p>Событий пока нет.</p>}</div>:null}</section>}

function Tasks({data,action}:{data:CrmData;action:(p:Record<string,unknown>)=>Promise<unknown>}){
  const [filter,setFilter]=useState("attention"),[now]=useState(()=>Date.now());
  async function create(event:FormEvent<HTMLFormElement>){event.preventDefault();const f=new FormData(event.currentTarget);await action({action:"create_task",title:f.get("title"),description:f.get("description"),priority:f.get("priority"),dueAt:f.get("dueAt"),reminderAt:f.get("reminderAt"),dealId:f.get("dealId"),customerId:f.get("customerId"),assignedTo:f.get("assignedTo")});event.currentTarget.reset();}
  const priorityLabel={low:"Низкая",normal:"Обычная",high:"Высокая",urgent:"Срочно"};
  const visible=data.tasks.filter(task=>filter==="all"||filter==="done"?Boolean(task.done):filter==="overdue"?!task.done&&new Date(task.due_at).getTime()<now:!task.done);
  return <><form className="admin-panel crm-task-create crm-task-create-detailed" onSubmit={create}><div><span>План работы</span><h2>Новая задача</h2><p>Назначьте сотрудника, срок, напоминание, срочность и связь с рабочим объектом.</p></div><label><span>Название</span><input name="title" required placeholder="Позвонить клиенту"/></label><label><span>Крайний срок</span><input name="dueAt" type="datetime-local" required/></label><label><span>Напомнить</span><input name="reminderAt" type="datetime-local"/></label><label><span>Срочность</span><select name="priority" defaultValue="normal"><option value="low">Низкая</option><option value="normal">Обычная</option><option value="high">Высокая</option><option value="urgent">Срочно</option></select></label><label><span>Клиент</span><select name="customerId"><option value="">Без клиента</option>{data.customers.map(c=><option value={c.id} key={c.id}>{c.name} · {c.phone}</option>)}</select></label><label><span>Сделка</span><select name="dealId"><option value="">Без сделки</option>{data.deals.map(d=><option value={d.id} key={d.id}>{d.title}</option>)}</select></label>{["owner","director"].includes(data.actor.role)?<label><span>Ответственный</span><select name="assignedTo" defaultValue={data.actor.id}>{data.staff.filter(s=>s.active).map(s=><option value={s.id} key={s.id}>{s.display_name}</option>)}</select></label>:null}<label className="full"><span>Описание</span><textarea name="description" rows={3} placeholder="Что нужно сделать и какой результат ожидается"/></label><button className="admin-primary" type="submit">Добавить задачу</button></form><div className="admin-panel crm-task-list crm-task-list-detailed"><div className="crm-task-filters"><button type="button" className={filter==="attention"?"active":""} onClick={()=>setFilter("attention")}>Требуют внимания</button><button type="button" className={filter==="overdue"?"active":""} onClick={()=>setFilter("overdue")}>Просрочено</button><button type="button" className={filter==="done"?"active":""} onClick={()=>setFilter("done")}>Завершено</button><button type="button" className={filter==="all"?"active":""} onClick={()=>setFilter("all")}>Все</button></div>{visible.map(task=>{const overdue=!task.done&&new Date(task.due_at).getTime()<now;return <label key={task.id} className={`${task.done?"done":""} ${overdue?"overdue":""} priority-${task.priority}`}><input aria-label={task.done?`Вернуть задачу «${task.title}»`:`Завершить задачу «${task.title}»`} type="checkbox" checked={Boolean(task.done)} onChange={()=>void action({action:"toggle_task",id:task.id,done:!task.done,version:task.version})}/><span><span className="task-meta"><b>{overdue?"Просрочено":priorityLabel[task.priority]}</b><small>{task.assigned_name}</small></span><strong>{task.title}</strong>{task.description?<p>{task.description}</p>:null}<small><Clock3 size={13}/>{new Date(task.due_at).toLocaleString("ru-RU")}{task.reminder_at?` · напоминание ${new Date(task.reminder_at).toLocaleString("ru-RU")}`:""}{task.customer_name?` · ${task.customer_name}`:""}{task.deal_title?` · ${task.deal_title}`:""}</small></span></label>})}{!visible.length?<Empty title="Задач в этом фильтре нет" text="Измените фильтр или добавьте новую задачу."/>:null}</div></>;
}
...
function StaffCard({person,staff,roleLabel,action}:{person:Staff;staff:Staff[];roleLabel:(value:string)=>string;action:(p:Record<string,unknown>)=>Promise<unknown>}){let initial:string[]|null=null;try{const parsed=JSON.parse(person.permissions_json) as unknown;if(Array.isArray(parsed))initial=parsed.filter((value):value is string=>typeof value==="string")}catch{initial=null}initial??=defaultRolePermissions[person.role]??[];const [permissions,setPermissions]=useState(initial),[reassignTo,setReassignTo]=useState(staff.find(item=>item.active&&item.id!==person.id)?.id??""),[confirmDelete,setConfirmDelete]=useState(false),[resetUrl,setResetUrl]=useState("");const protectedRole=["owner","director"].includes(person.role);async function createReset(){const response=await fetch("/api/admin/security",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"create_reset",staffId:person.id})}),body=await response.json() as {url?:string;error?:string};if(!response.ok){setResetUrl(body.error??"Не удалось создать ссылку");return}setResetUrl(body.url??"")}return <article className="staff-card"><i><ShieldCheck/></i><div className="staff-card-main"><strong>{person.display_name}</strong><span>{person.email}</span><small>{[roleLabel(person.role),person.position,person.department].filter(Boolean).join(" · ")}</small>{person.skills?<p>Навыки: {person.skills}</p>:null}{person.bio?<p>{person.bio}</p>:null}{person.active?<button type="button" onClick={()=>void createReset()}>Создать ссылку восстановления</button>:null}{resetUrl?<div className="invite-result"><code>{resetUrl}</code>{resetUrl.startsWith("http")?<button type="button" onClick={()=>void navigator.clipboard.writeText(resetUrl)}><Clipboard size={16}/>Копировать</button>:null}</div>:null}</div>{protectedRole?<b>{roleLabel(person.role)}</b>:<details className="staff-access"><summary>Доступ и действия</summary><PermissionPicker value={permissions} onChange={setPermissions}/><label><span>Передать активную работу</span><select value={reassignTo} onChange={event=>setReassignTo(event.target.value)}>{staff.filter(item=>item.active&&item.id!==person.id).map(item=><option value={item.id} key={item.id}>{item.display_name}</option>)}</select></label><div className="team-actions"><button type="button" onClick={()=>void action({action:"staff_permissions",id:person.id,permissions})}>Сохранить доступ</button><button className={person.active?"danger":""} type="button" onClick={()=>void action({action:"staff_active",id:person.id,active:!person.active,reassignTo})}>{person.active?"Заблокировать и передать":"Разблокировать"}</button>{confirmDelete?<><span>История сохранится, доступ будет удалён.</span><button className="danger" type="button" onClick={()=>void action({action:"staff_delete",id:person.id,reassignTo})}>Подтвердить удаление</button><button type="button" onClick={()=>setConfirmDelete(false)}>Отмена</button></>:<button className="danger" type="button" onClick={()=>setConfirmDelete(true)}>Удалить доступ</button>}</div></details>}</article>}
...
const auditLabels:Record<string,string>={create_deal:"Создана сделка",update_deal:"Сделка обновлена",add_note:"Добавлена заметка",create_task:"Создана задача",toggle_task:"Изменён статус задачи",save_cost:"Обновлена себестоимость",staff_active:"Изменён доступ сотрудника",staff_delete:"Удалён доступ сотрудника",invite_manager:"Создано приглашение",manager_joined:"Сотрудник принял приглашение",save_preferences:"Обновлён профиль",update_customer:"Обновлена карточка клиента",create_lead:"Создана заявка",update_lead:"Заявка обновлена",archive_record:"Запись архивирована"};
const bishkekDate=(value:string)=>new Intl.DateTimeFormat("ru-RU",{dateStyle:"medium",timeStyle:"short",timeZone:"Asia/Bishkek"}).format(new Date(value));
function AuditLog({rows}:{rows:Audit[]}){return <div className="admin-panel crm-audit"><div className="panel-head"><div><span>Безопасность</span><h2>Журнал действий</h2></div></div>{rows.map(row=><article key={row.id}><time>{bishkekDate(row.created_at)}</time><strong>{row.display_name||"Система"}</strong><span>{auditLabels[row.action]??"Изменение записи"}</span><p>{row.detail&&!/^\s*[{[]/.test(row.detail)?row.detail:"Данные объекта обновлены"}</p></article>)}{!rows.length?<Empty title="Событий пока нет" text="Изменения сотрудников будут записываться здесь."/>:null}</div>}
